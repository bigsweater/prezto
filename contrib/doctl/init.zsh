...
_doctl()
{
    ...
    while IFS='\n' read -r comp; do
        ...
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            __doctl_debug "ActiveHelp found: $comp"
            comp="${comp[$startIndex,-1]}"
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __doctl_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __doctl_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveKeepOrder)) -ne 0 ]; then
        __doctl_debug "Activating keep order."
        keepOrder="-V"
    fi
